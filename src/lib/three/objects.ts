...
export const addGrid = (scene: THREE.Scene, size: number, divisions: number, color: number) => {
  const gridHelper = new THREE.GridHelper(size, divisions, color, color);
  gridHelper.name = 'grid';
  scene.add(gridHelper);
};

export const addGridBox = (scene: THREE.Scene, size: number, divisions: number, color: number) => {
  const gridHelper = new THREE.GridHelper(size, divisions, color, color);
